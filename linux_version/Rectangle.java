...
class Rectangle
{
	...
	public Rectangle(Stack<ConnectPoints> l)
	{
		line=l;
		if(l.size()>0)
		{
	...
	public Rectangle()
	{
		line=new Stack<ConnectPoints>();
	}

	public void addM(ConnectPoints l)
	{
		if (l!=null)
		{
			line.add(l);
	...
	{
		double x,y;
		x=line.elementAt(0).min.x;
		y=line.elementAt(0).min.y;
		for (ConnectPoints l: line)
		{
			if (l.min.x<x)
				x=l.min.x;
			if (l.min.y<y)
	...
	{
		double x,y;
		x=line.elementAt(0).max.x;
		y=line.elementAt(0).max.y;
		for (ConnectPoints l: line)
		{
			if (l.max.x>x)
				x=l.max.x;
	...
	private boolean cohenSutherland()
	{
		...
		{
			Point p1=line.elementAt(i).pDepart.clone();
			Point p2=line.elementAt(i).pArrivee.clone();
			c1=code(p1);
			c2=code(p2);
			while((c1|c2)!=0 && (c1&c2)==0)
			{
				...
				if ((c1&1)==1)
				{
					p1.x=line.elementAt(i).xCorr(min.y);
					p1.y=min.y;
				}
				else if ((c1&2)==2)
				{
					p1.x=line.elementAt(i).xCorr(max.y);
					p1.y=max.y;
				}
				else if ((c1&4)==4)
				{
					p1.x=max.x;
					p1.y=line.elementAt(i).yCorr(max.x);
				}
				else
				{
					p1.x=min.x;
					p1.y=line.elementAt(i).yCorr(min.x);
				}
			c1=code(p1);
			}
			if(c1!=0 || c2!=0)
	...
	{
		Stack<ConnectPoints> l=new Stack<ConnectPoints>();
		double minx=min.x+(max.x-min.x)/2;
		double miny=min.y+(max.y-min.y)/2;
		for (ConnectPoints ln:line)
	...
	public Rectangle clone()
	{
		...
		r.line=ConnectPoints.vect_clone(line);
		r.min=min.clone();
		r.max=max.clone();
		return r;
	}
